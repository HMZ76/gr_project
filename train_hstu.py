...
# 确保你的 dataset 文件名正确，包含最新的 hstu_collate_fn 修复
from dataset import AmazonDataset, hstu_collate_fn, hstu_eval_collate_fn
...
def evaluate(model, dataloader, device, k_list=None):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating", leave=False):
            ...
            # Forward pass
            logits, _ = model(input_ids, timestamps)
...
def train_epoch(model, dataloader, optimizer, scheduler, device, use_sampled_softmax=False):
    ...
    for batch in tqdm(dataloader, desc="Training", leave=False):
        ...
        if use_sampled_softmax:
            _, loss = model.forward_sampled_softmax(input_ids, timestamps, targets)
        else:
...
def main():
    parser = argparse.ArgumentParser(description='Train HSTU model')
    ...
    parser.add_argument('--use_temporal_bias', action='store_true', help='Use temporal attention bias')
    ...
    # Note: Make sure AmazonHSTUDataset is correctly imported, or use AmazonDataset if you renamed it
    ...
    collate_train = lambda x: hstu_collate_fn(x, max_seq_len=args.max_seq_len)
    collate_eval  = lambda x: hstu_eval_collate_fn(x, max_seq_len=args.max_seq_len)
    ...
    for epoch in range(args.epochs):
        ...
        # 🚨 FIX: Evaluate on validation set during training, not test set!
        ...
        if valid_ndcg_k > best_valid_ndcg:
            ...
            checkpoint_path = os.path.join(args.checkpoint_dir, f'hstu_{args.dataset}_best.pth')
            torch.save({
                'epoch': epoch,
